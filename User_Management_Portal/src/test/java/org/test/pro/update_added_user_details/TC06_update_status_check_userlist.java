...
public class TC06_update_status_check_userlist extends BaseClass {
	...
	public void update_user_details() throws IOException, InterruptedException {
		...
		status.click();// status change to inactive
		po.getSavebutton().click();	
		sleep();
		String userstatus = driver.findElement(By.xpath("//span[text()='"+u+"']/following::label[1]")).getText();
		System.out.println(userstatus);
		if (userstatus.equalsIgnoreCase(XLdata.getStringCellData(path, sheet1, 7, 6))) 
		{
				
				System.out.println("TC06 - updated user status properly visibel in usermanagement user list");
				XLdata.pass(path, sheet2, 6, 2);
		} 
		else 
		{
			System.out.println("TC06 - updated user status NOT properly visibel in usermanagement user list");
