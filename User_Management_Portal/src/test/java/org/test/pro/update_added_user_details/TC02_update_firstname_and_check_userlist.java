...
public class TC02_update_firstname_and_check_userlist extends BaseClass {
	...
	public void update_user_details() throws IOException, InterruptedException {
		...
		firstname.clear();
		firstname.sendKeys(XLdata.getStringCellData(path, sheet1, 8, 4));
		...
		if (usermail== true) 
		{
			String username = driver.findElement(By.xpath("//span[text()='"+u+"']/preceding::span[2]")).getText();//username field
			if ((username.equals("Udemo user1"))) 
			{
				System.out.println("TC02 - updated user firstname properly visibel in usermanagement user list");
				XLdata.pass(path, sheet2, 2, 2);
			} 
			else 
			{
				System.out.println("TC02 - updated user firstname NOT properly visibel in usermanagement user list");
