...
//project purpose : Add System User by disable "Admin" option and check whether the user has admin privilege or not.
public class TC12_check_nonadmin_user_privileges extends BaseClass{
	...
	public void add_system_user() throws IOException, InterruptedException {
		...
		System.out.println("TC12 - Check admin user privileges");
...
//		po.getAdminbutton().click();// admin enable
		...
		if (usertype.equals(XLdata.getStringCellData(path, sheet1, 13, 7))) 
		{
			sleep();
			signout();
			sleep();
...
//		po.getAdminbutton().click();// admin enable
		...
		String usertype = driver.findElement(By.xpath("//span[text()='"+u+"']/following::span[2]")).getText();
		if (usertype.equals(XLdata.getStringCellData(path, sheet1, 14, 7))) 
		{
			sleep();
			signout();
			sleep();
...
//					""};
			...
			if (i == options.size()) 
			{
				...
				XLdata.pass(path, sheet2, 12, 3);
			} 
			else {
				XLdata.fail(path, sheet2, 12, 3);
...
//		po.getAdminbutton().click();// admin enable
		...
		String usertype = driver.findElement(By.xpath("//span[text()='"+u+"']/following::span[2]")).getText();
		if (usertype.equals(XLdata.getStringCellData(path, sheet1, 15, 7))) 
		{
			sleep();
			signout();
			sleep();
...
//					""};
			...
			if (i == options.size()) 
			{
				...
				XLdata.pass(path, sheet2, 12, 4);
			} 
			else {
				XLdata.fail(path, sheet2, 12, 4);
