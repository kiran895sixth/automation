...
//project purpose :  Check whether the Active user can able to login or not

public class TC15_check_active_user_able_to_login extends BaseClass {
	...
	public void add_system_user() throws IOException, InterruptedException {
		...
		if (status.equalsIgnoreCase(XLdata.getStringCellData(path, sheet1, 9, 6))) 
		{
			...
			po.getSigninbutton().click();
			sleep();
			if (driver.findElement(By.xpath("//label[text()='Password *']/following::input[1]")) != null) 
			{
				System.out.println("TC15 - Active user can able login ");
				XLdata.pass(path, sheet2, 15, 2);
			} 
			else 
			{
				System.out.println("TC15 - Active user can't  able login ");
	...
	public void add_system_user1() throws IOException, InterruptedException {
		...
		String status = driver.findElement(By.xpath("//span[text()='"+u+"']/following::label[1]")).getText();
		if (status.equalsIgnoreCase(XLdata.getStringCellData(path, sheet1, 10, 6))) 
		{
			signout();
			sleep();
			...
			{
				System.out.println("TC15 - Active user can able login ");
				XLdata.pass(path, sheet2, 15, 3);
			} 
			else 
			{
				System.out.println("TC15 - Active user can't  able login ");
				XLdata.fail(path, sheet2, 15, 3);
			}
		} 
		else 
		{
	...
	public void add_system_user2() throws IOException, InterruptedException {
		...
		if (status.equalsIgnoreCase(XLdata.getStringCellData(path, sheet1, 11, 6))) 
		{
			signout();
			sleep();
			...
			{
				System.out.println("TC15 - Active user can able login ");
				XLdata.pass(path, sheet2, 15, 4);
			} 
			else 
			{
				System.out.println("TC15 - Active user can't  able login ");
				XLdata.fail(path, sheet2, 15, 4);
			}
		} 
		else 
		{
